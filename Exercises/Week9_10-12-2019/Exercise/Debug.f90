	...
	INTERFACE Checkpoint
		MODULE PROCEDURE CP_matrix_real8, CP_matrix_comp8, CP_matrix_comp16, CP_vec_real8, CP_vec_comp16
	END INTERFACE
	
	INTERFACE Dump_matrix
		MODULE PROCEDURE Dump_matrix_
	END INTERFACE
	...
	SUBROUTINE Dump_matrix_(matrix_)
		! Subroutine to print to screen the matrices in a clear form
		IMPLICIT NONE

		COMPLEX*16, DIMENSION(:,:), INTENT(IN) :: matrix_
		INTEGER*4 :: ii,jj
		
		DO ii=1,SIZE(matrix_,1)
			WRITE(12,"(F5.2,F7.2)",advance="No") matrix_(ii,1)

			DO jj=2,SIZE(matrix_,2)-1
				WRITE(12,"(F7.2,F7.2)",advance="No") matrix_(ii,jj)
			END DO
		
			WRITE(12,"(F7.2,F7.2)",advance="Yes") matrix_(ii,SIZE(matrix_,2))
		END DO
	
	END SUBROUTINE Dump_matrix_
	...
	SUBROUTINE Print_error(Debug, Error)
		...
		IF (Debug) THEN
			...
			CASE("Num_partic<=0")
				WRITE(*,*) "ERROR: number of paricles should be greater than 0."
				STOP
	...
	! Writing CHECKPOINT subroutine for COMPLEX*16 MATRICES
	...
	! Writing CHECKPOINT subroutine for COMPLEX*16 VECTORS
	SUBROUTINE CP_vec_comp16(Debug, IO, label, comp_vec, dim_)
		! Subroutine used to save/retrieve stuff from file
		! Arguments are:
		!	Debug		: logical, flag which confirms the call for debugging
		!	IO		: string, should be "Read" or "Write", tells if read or write on log file
		!	label		: string. File names are: ".Status_" + label + ".log"
		!	comp_vec	: comp*8 1D array (in this specific case); is a variable to be stored
		!	dim_		: integer*8. Used to eventually print a 0s vector in the file.
		!				dims should correspond to actual comp_vec dimension.
		...
		CHARACTER(50) :: file_name
		INTEGER*8, INTENT(IN) :: dim_
		COMPLEX*16, DIMENSION(dim_), INTENT(INOUT) :: comp_vec
		INTEGER*8 :: stat, ii, counter
		COMMON counter
		...
		IF (Debug) THEN	
			...
			IF (IO.EQ."Read") THEN
				...
				IF (stat.EQ.0) THEN
					READ(11,"(F13.10,F13.10)") comp_vec
				ELSE
					! If no data to retrieve, then start from scratch
					DO ii=1,dim_
						comp_vec(ii)=CMPLX(0d0,0d0)
					END DO
				END IF
			...
			IF (IO.EQ."Write") THEN
				...
				! Writing data to file
				DO ii=1,dim_
					WRITE(11,"(F13.10,F13.10)", advance="No") comp_vec(ii)
				END DO
				WRITE(11,"(F13.10,F13.10)") comp_vec(dim_)
				...
				WRITE(11,*) ""
				DO ii=1,dim_
					WRITE(11,"(F15.10,F15.10)", advance="No") comp_vec(ii)
				END DO
				!WRITE(11,*) comp_vec
